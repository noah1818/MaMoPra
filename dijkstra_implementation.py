...
class Dijkstra():
    ...
    def dijkstra(self, graph: nx.DiGraph, start: int, end: int, attribute) -> Tuple[Optional[list[int]], float]:
        ...
        while priority_queue:
            ...
            if current_node == end:
                ...
                return path[::-1], cum_atttribute, visited_nodes_count
        ...
        return None, float("inf"), visited_nodes_count

    def get_shortest_path(self, graph: nx.DiGraph, start: int, goal: int, attribute: str) -> Tuple[Optional[list[int]], float]:
        path, distance, visited_nodes_count = self.dijkstra(graph, start, goal, attribute)
        if path is None or path[0] != start:
            # No valid path found
            return None, float("inf"), visited_nodes_count
        return path, distance, visited_nodes_count
